...
//import("apps/view/inc/sidebar.php"); 
?>
<script src="/<?php echo STATIC_URL; ?>/orgchart.js"></script>
<style>
html, body{
  width: 100%;
  height: 100%;
...
if (authenticate() == true) {
	...
	$tree = $pvctrl->d3_tree($_SESSION['user_id']);
}
?>
<section>
	...
	<script>
		...
		const nestedData = JSON.parse(`<?php echo json_encode($tree); ?>`);

		function flatten(data, parentId = null) {
            let flattened = [];
            data.forEach(item => {
                flattened.push({ id: item.id, pid: parentId, name: item.name, tags: ["yellow"], img: 'https://cdn.balkan.app/shared/7.jpg' });
                if (item.children) {
                    flattened = flattened.concat(flatten(item.children, item.id));
                }
            });
            return flattened;
        }
