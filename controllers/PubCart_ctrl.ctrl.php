...
class PubCart_ctrl
{
    public function add_or_remove($action = 'add')
    {
        ...
        $cart = new Dbobjects;
        $pv = obj(getData('item', $req->item_id));
        $sql = "select id, qty, item_id from customer_order where user_id=$guestid";
        ...
        if (in_array(MY_COUNTRY, $cntrs)) :
            ...
            // $cart = new Dbobjects;
            $sql = "select qty from customer_order where item_id = $req->item_id and status='cart' and user_id=$guestid";
            $old = $cart->show($sql);
            if ($action == 'add') :
                if (count($old) > 0) {
                    $sql = "update customer_order set qty = (qty+1), jsn = '$update_jsn' where item_id = $req->item_id and status='cart' and user_id=$guestid";
                    $pdo = $cart->dbpdo();
                    ...
                    ($req->item_id,  $guestid, 1, $price, $tax, $mypv, $myrv, $mybonus, 'cart', '$update_jsn')";
                    // $cart = new Dbobjects;
                    $pdo = $cart->dbpdo();
                    $stmt = $pdo->prepare($sql);
                    $stmt->execute();
            ...
            elseif ($action == 'remove') :
                ...
                if (count($old) > 0) {
                    if ($old[0]['qty'] == 1) {
                        $sqldel = "delete from customer_order where item_id = $req->item_id and status='cart' and user_id=$guestid";
                        $pdo = $cart->dbpdo();
                        $stmt = $pdo->prepare($sqldel);
                        $stmt->execute();
                    } else {
                        $sqlup = "update customer_order set qty = (qty-1), jsn = '$update_jsn' where item_id = $req->item_id and status='cart' and user_id=$guestid";
                        $pdo = $cart->dbpdo();
